...
# Define the offer box position based on text4
offer_box_width = 300
...
# Add space between the offer box and the square box
space_between_elements = 20  # Adjust the desired space
get_box_width = 100
get_box_height = offer_box_height
get_box_x = offer_box_x - get_box_width - space_between_elements  # Adjust the spacing
get_box_y = offer_box_y
get_box_color = (255,255,255)  # Change the background color to black for visibility

# Create a square box on the left side of the offer box
draw.rectangle([get_box_x, get_box_y, get_box_x + get_box_width, get_box_y + get_box_height], fill=get_box_color)

# Add text "(Get)" inside the square box
get_text = "Get"
get_font_size = 40
get_font = ImageFont.truetype(font_path_title, size=get_font_size)
get_text_color = (0,0,0)  # Change the text color to white for visibility

# Calculate the position to center the text inside the square box
get_text_mask = get_font.getmask(get_text)
get_text_x = get_box_x + (get_box_width - get_text_mask.getbbox()[2]) // 2
get_text_y = get_box_y + (get_box_height - get_text_mask.getbbox()[3]) // 2
#get text in vertical
draw.text((get_text_x, get_text_y), get_text, fill=get_text_color, font=get_font,text_direction="vertical")
# Add the text "(Get)" inside the square box
# draw.text((get_text_x, get_text_y), get_text, fill=get_text_color, font=get_font,text_direction="vertical")
